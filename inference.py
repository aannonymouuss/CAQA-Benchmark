...
class InferenceArguments:
    ...
    load_in_8bit: bool = field(
        default=False,  #######  True when use LLama-70B
    ...
    # prompt_type: str = field(default="attribution-no-definition-w-examples", metadata={"help": "prompt engineering: which prompt to use"})
    # prompt_type: str = field(default="attribution2category-no-definition", metadata={"help": "prompt engineering: which prompt to use"})
...
def inference():
    ...
    if tokenizer.pad_token is None:
        smart_tokenizer_and_embedding_resize(
            special_tokens_dict=dict(pad_token=DEFAULT_UNK_TOKEN),
            tokenizer=tokenizer,
            model=model,
        )
    ...
    if model_name == "t5_xxl_true_nli_mixture":
        inference_args.prompt_type = "ALCE"
    elif model_name.startswith("attrscore-"):
        inference_args.prompt_type = "AttScore"
    else:
        inference_args.prompt_type = inference_args.prompt_type
    ...
    for ii, datum in enumerate(tqdm(test_data)):
        ...
        # outputs = model.generate(input_ids=inputs["input_ids"].to(f'cuda:{gpu_device}'),
        ...
        # input()
        if inference_args.prompt_type  == "attribution2category-no-definition":
            if datum["label"] in ['Missing', 'Contradictory', 'Irrelevant']:
                datum["label"] = 'Unsupported'
            else:
                datum["label"] = 'Supportive'
        # print("Output:",prediction)
        # print("Prediction:",pred_label)
        # print("GOLD:",datum["label"])
        # print()
        # input()
    ...
    if inference_args.prompt_type  == "attribution2category-no-definition":
        conf_matrix = confusion_matrix(true_labels, pred_labels, labels=["Supportive", "Unsupported"])
    else:
        conf_matrix = confusion_matrix(true_labels, pred_labels, labels=["Support", "Missing", "Contradictory", "Irrelevant"])
